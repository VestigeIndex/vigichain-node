...
pub fn apply_baseline(command: &mut Command, paths: &SandboxPaths) {
    ...
    if let Some(v) = system_root {
        command.env("SYSTEMROOT", v);
    }
    if let Some(v) = windir {
...
pub fn capability() -> (&'static str, &'static str) {
    #[cfg(unix)]
    {
        return (
            "os-constrained",
            "private dirs + cleared environment + no_new_privs + core-dump disabled",
        );
    }
    #[cfg(windows)]
    {
        return ("os-constrained","private dirs + cleared environment + Job Object process-tree containment + hard CPU cap");
    }
    #[allow(unreachable_code)]
    ("data-isolated", "private dirs + cleared environment")
}
