...
fn local_endpoint() -> Result<String, String> {
    ...
    let parsed =
        reqwest::Url::parse(&endpoint).map_err(|e| format!("Invalid telemetry URL: {e}"))?;
    if parsed.scheme() != "http" {
        return Err("Telemetry fallback must use local HTTP only".into());
    }
...
pub fn snapshot() -> Result<TelemetryState, String> {
    ...
    let response = match client.get(&endpoint).send() {
        ...
        Err(e) => {
            return Ok(TelemetryState {
                available: false,
                endpoint,
                snapshot: None,
                reason: Some(format!("Core telemetry unavailable: {e}")),
            })
        }
    };
    if !response.status().is_success() {
        return Ok(TelemetryState {
            ...
            snapshot: None,
            reason: Some(format!(
                "Core telemetry returned HTTP {}",
                response.status()
            )),
        });
    }
    let snapshot: CoreTelemetry = response
        .json()
        .map_err(|e| format!("Core telemetry schema is invalid: {e}"))?;
    if snapshot.schema_version != 1 {
        return Err(format!(
            "Unsupported Core telemetry schema {}",
            snapshot.schema_version
        ));
    }
    if snapshot.network != "testnet" && snapshot.network != "mainnet" {
        return Err("Telemetry returned an unknown VigiChain network".into());
    }
    if !(0.0..=1.0).contains(&snapshot.sync.progress) {
        return Err("Telemetry sync progress is outside 0..1".into());
    }
    Ok(TelemetryState {
        available: true,
        endpoint,
...
mod tests {
    ...
    fn default_is_loopback() {
        assert!(DEFAULT_ENDPOINT.starts_with("http://127.0.0.1:"));
    }
}
